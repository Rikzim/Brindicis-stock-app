<script lang="ts">
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      } else if (crud.selected?.id != null) {
        await updateFamily(crud.selected.id, {
      ...
      refetchFamilies();
      toast.success(
        crud.isAdd ? "Família adicionada com sucesso." : "Família atualizada com sucesso."
      );
      crud.close();
    } catch {
  ...
  let familyProducts = $derived.by(() => {
    const sel = crud.selected;
    if (!sel?.name) return [];
    const id = sel.id;
    const nameUpper = sel.name.toUpperCase();
    return (productsStore.data || []).filter(
      (p) => p.familyId === id || p.family?.name.toUpperCase() === nameUpper
    );
  });
...
>
  ...
  >
    ...
    >
      <Plus class="size-3.5" /><span>Adicionar Família</span>
    </Button>
  </div>
    ...
    {#snippet cell(row, col)}
      ...
      {:else if col.key === "products"}
        <span class="text-center font-medium"
          >{productCountsByFamily[row.name.toUpperCase()] || 0}</span
        >
            ...
            {#each familyProducts as prod (prod.id)}
              <div class="p-3 flex items-center justify-between text-sm">
                ...
                <span
                  ...
                  >Stock: {prod.quantity}</span
                >
              </div>
            {/each}
          </div>
        {:else}
          <span class="text-sm text-slate-500 italic">Nenhum produto cadastrado nesta família.</span
          >
        {/if}
      </div>
    </AdminModal>
